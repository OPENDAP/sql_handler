...
protected:
    SQLInternalError() =default;

public:
    SQLInternalError(std::string msg, std::string file, unsigned int line)
        : BESError(std::move(msg), _SQLH_INTERNAL_ERROR, std::move(file), line) { }

    ~SQLInternalError() override = default;
