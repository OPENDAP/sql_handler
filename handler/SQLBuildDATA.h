...
public:
	 ...
	 */
static bool
sql_build_data( SQLDataHandlerInterface &dhi ,
...
sql_build_data( SQLDataHandlerInterface &dhi,
		...
		SQLSimpleConnector<SQL_TYPE,ODBC_TYPE>* connector )
	{
		...
		SQLQueryAction<ERROR_TYPE,MSG_TYPE,OUT_TYPE1>::
			query(*connector,error_factory);
		...
		if (!bdds)
			throw BESInternalFatalError(
				"SQLBuildDATA: Unable to get DataDDSResponse object",
					__FILE__,__LINE__);
		DataDDS *dds = bdds->get_dds();

        SQLSequence<SQL_TYPE,ODBC_TYPE> *seq=
			new SQLSequence<SQL_TYPE,ODBC_TYPE>(
				dhi.getBesContainer()->get_symbolic_name(),
				connector->getParams().get_real_name(),
				connector);
		...
		for (size_t i=0; i<connector->getCols(); i++){
			...
			{
				BESDEBUG(SQL_NAME,"SQLBuildDATA: : Unrecognized NULL object. "
 ...
 * setNext don't know how long to go...
				...
				if (bt) {
					...
					throw BESInternalError(
						"Unable to allocate SQLObject",__FILE__,__LINE__);
			}
		} // endfor col
...
TESTDEBUG(SQL_NAME_TEST,"------ATTR DUMP DONE------"<<endl);
			...
			throw BESInternalError(
				"Unable to build an SQLSequence",__FILE__,__LINE__);
		...
		if (das){
			...
			/**
			 * @todo: check this
			 * comments says that it is safe to delete (copied)
			 * but seams that it's not true!
			 *
			 * from DDS.cc 358-359
...
//			das=0;
			...
			throw BESInternalFatalError(
				"SQLBuildDATA: Unable to transfer a NULL DAS object",
				__FILE__,__LINE__);
#endif

		dhi.getBesHandler().data[POST_CONSTRAINT] =
				dhi.getBesContainer()->get_constraint();
