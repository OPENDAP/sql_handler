...
public:
	 ...
	 */
static bool
sql_build_dds( SQLDataHandlerInterface &dhi ,
		...
		SQLSimpleConnector<SQL_TYPE,ODBC_TYPE>* connector )
	{
		...
		if (!bdds)
			throw BESInternalFatalError(
				"SQLBuildDDS: Unable to get DDSResponse object",
					__FILE__,__LINE__);
		...
		for (size_t i=0; i<connector->getCols(); i++){
			...
			{
				BESDEBUG(SQL_NAME,"SQLBuildDDS: : Unrecognized NULL object. "
 ...
 * which HERE may fail.
		...
		if (seq){
			...
			BESDEBUG(SQL_NAME,"SQLBuildDDS: set dataset name"<<endl);

			dds->set_dataset_name(dhi.getBesContainer()->get_real_name());
			dds->filename(dhi.getBesContainer()->get_real_name());

#if __TESTS__==1
TESTDEBUG(SQL_NAME,"--------------DATA DUMP---------------"<<endl);
attr.dump(std::cerr);
TESTDEBUG(SQL_NAME,"------------SEQ DUMP DONE------------"<<endl);
#endif
		}
		else
			throw BESInternalError(
