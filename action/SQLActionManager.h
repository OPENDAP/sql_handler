...
public:
	...
	static _JOIN* doActions(SQLActionFactory<CODE_TYPE, ARGS_TYPE, OUT_TYPE> &_af, MERGE merge_f = &merge, JOIN join_f =
			&join, bool multiple_args = true)
	{
		if (!merge_f) merge_f = &merge;
		if (!join_f) join_f = &join;
...
private:
	...
	static OUT_TYPE * tryNext(SQLActionList<ARGS_TYPE, OUT_TYPE> &actions, ARGS_TYPE *args)
	{
		try {
