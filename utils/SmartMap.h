...
struct less: public std::binary_function<_St, _St, bool> {
	bool operator()(const _St& __x, const _St& __y) const
	{
		return (*(__x) < *(__y));
	}
};
...
public:
	 ...
	 */
	inline std::pair<iterator, bool> insert(K &k, V &v)
	{
		return _map.insert(std::make_pair(T1(k), T2(v)));
	}
	 ...
	 */
	inline size_t erase(K &k)
	{
		return _map.erase(T1(k));
	}

	inline void erase(SmartMap::iterator k)
	{
		return _map.erase(k);
	}

	inline iterator find(K *k)
	{
	...
	inline const_iterator find(K *k) const
	{
		return _map.find(T1(k));
	}
...
public:
	...
	inline std::pair<iterator, bool> insert(std::pair<K, V> &__x)
	{
		return _map.insert(std::make_pair(__x.first, T(__x.second)));
	}

	inline std::pair<iterator, bool> insert(K &k, V &v)
	{
		return _map.insert(std::make_pair(k, T(v)));
	}
	...
	inline void erase(SmartValueMap::iterator k)
	{
		/*return _map.erase(k);jhrg 10/1/14*/
		_map.erase(k);
	}
